---
title: BY-COVID
description: The BeYond-COVID (BY-COVID) project aims to provide open data on SARS-CoV-2 and other infectious diseases across scientific, medical and public health domains.
class: home
imageTwitter: /twitter-home.jpg
imageOg: "/og-home.jpg"
imageAlt: Image for the BY-COVID website.
---

import { StaticImage } from "gatsby-plugin-image"
import { Container, Row, Col, Card } from "react-bootstrap"
import { Link } from "gatsby"
import NewsletterForm from "../components/newsletter-form"
import { navigate } from "gatsby"
import { TwitterTimelineEmbed } from 'react-twitter-embed';

<div class="w-60 text-center home-intro">

<div class="intro-container">

The BeYond-COVID(2021–2024) project aimed to make COVID-19 data accessible to scientists in laboratories but also to anyone who can use it, such as medical staff in hospitals or government officials.

Going beyond SARS-CoV-2 data, the project aimed to provide a framework for making data from other infectious diseases open and accessible to everyone.

Project outcomes are summarised in our <a href="https://by-covid.org/pdf/update-elixir-by-covid-brochure-1-7-DIGITAL.pdf" style="color: #ffffff; text-decoration: underline;">Success Stories brochure</a>.

</div>

<!-- The Survey -->

<!-- The challenge -->

## The challenge

The world has generated vast amounts of data in response to the COVID-19 pandemic, and is still generating more. This data comes from many different sources, and identifying, connecting and integrating it for effective analysis remains a challenge.

</div>
<div>

<Container className="lh-sm data-sources big-top-space">
  <Row>
    <Col className="data-source hospitals">
      <div>Hospitals</div>
    </Col>
    <Col className="data-source medical">Medical Centres</Col>
    <Col className="data-source public">Public Health Laboratories</Col>
    <Col className="data-source research-infrastructures">
      Research Infrastructures
    </Col>
    <Col className="data-source universities">Universities</Col>
  </Row>
  <Row>
    <Col className="data-source consortia">Consortia</Col>
    <Col className="data-source research-centres">Research Centres</Col>
  </Row>
</Container>

<div class="data-lines text-center fs-6">
<p>SARS-CoV-2 and other infectious disease data comes from many sources</p>

</div>

<!-- What we are doing -->

<div class="bg-semi-opaque what-doing container full-width text-body text-center">
  <div class="w-60 text-center">

## What we are doing

<div class="what-doing-boxes">
  <div class="box box1 bg-white text-body">
    <h3 class="fs-6 fw-bold mb-1 text-primary">1. Mobilising data</h3>
    <p>
      Ensuring data from around the world can easily be submitted to core data
      hubs.
    </p>
  </div>
  <div class="box box2 bg-white text-body">
    <h3 class="fs-6 fw-bold mb-1 text-primary">2. Connecting data</h3>
    <p>
      Linking infectious disease data. For instance, clinical data with viral
      data.
    </p>
  </div>
  <div class="box box3 bg-white text-body">
    <h3 class="fs-6 fw-bold mb-1 bg-white text-primary">
      3. Standardising data
    </h3>
    <p>
      Providing recommended data management best practices and standards for
      describing data.
    </p>
  </div>
  <div class="box box4 bg-white text-body">
    <h3 class="fs-6 fw-bold mb-1 bg-white text-primary">
      4. Exposing and analysing data
    </h3>
    <p>Providing data analysis methods and protocols.</p>
  </div>
</div>

<div class="halo-container">

<div class="covid-portal">
<h2 class="visually-hidden-focusable">COVID-19 Portal</h2>

<p class="mb-0">
  Making the data findable in the European COVID-19 Data Portal, which will
  connect existing virus data resources.
</p>
<p class="mb-0">
  The Portal allows entry to open research data and access-controlled patient
  data.
</p>
<p class="mb-0 mt-2">
  <a href="/about#data-platform" aria-label="Read more about Data Portal">
    Read more
  </a>
</p>

</div>
</div>
</div>

</div>

</div>

<!-- The outcomes -->

  <div class="w-60 text-center outcomes-box">
  <div class="outcomes-text">
<h2 class="mt-3">The outcomes</h2>

The project will make data about SARS-CoV-2 and other viruses easier to access, aggregate and analyse. This will enable scientists to respond faster to new strains of SARS-CoV-2 or to new viruses. It will also help policy-makers assess the impact of the disease and take the most appropriate measures to protect people against new viral diseases.

</div>
</div>

<div class="bg-semi-opaque container full-width big-top-space text-body px-4 ">

<Container className="twitter">
  <Row>
    <Col className="align-items-stretch mb-4 mb-md-0 col-center" xs={12} sm={6} lg={8}>
      <Card className="mb-4">
        <Card.Body className="position-relative">
          <Card.Title as="h3" className="mt-3 fw-bold fs-4 lh-sm">
            About
          </Card.Title>
          <Card.Text className="mb-4 mt-3 px-0">
            Discover the objectives, the structure of the project and the
            project partners.
          </Card.Text>
          <div class="bottom-link-right arrow-right">
            <Link to="/about">About</Link>
          </div>
        </Card.Body>
      </Card>
      <Card className="mb-4">
        <Card.Body className="position-relative">
          <Card.Title as="h3" className="mt-3 fw-bold fs-4 lh-sm">
            News & events
          </Card.Title>
          <Card.Text className="mb-4 mt-3 px-0">
            See the latest news, events and announcements.
          </Card.Text>
          <div class="bottom-link-right arrow-right">
            <Link to="/news">News</Link> & <Link to="/events">events</Link>
          </div>
        </Card.Body>
      </Card>
       <Card className="mb-4">
        <Card.Body className="position-relative">
          <Card.Title as="h3" className="mt-3 fw-bold fs-4 lh-sm">
            Outreach & media
          </Card.Title>
          <Card.Text className="mb-4 mt-3 px-0">
            The project logos, graphics and outreach activities.
          </Card.Text>
          <div class="bottom-link-right arrow-right">
            <Link to="/outreach-media">Outreach & media</Link>
          </div>
        </Card.Body>
      </Card>
                      <h2 class="fs-3 lh-2 mb-2 ps-0 text-dark">Want to learn more?</h2>
      <p class="ps-0 text-dark">Subscribe to our newsletter:</p>
      <NewsletterForm />
    </Col>
    {/*<Col className="align-items-stretch mb-4 mb-md-0" xs={12} sm={6} lg={3}>
      <Card className="mb-4 home-twitter-box twitter-embed">
         <TwitterTimelineEmbed
          sourceType="profile"
          screenName="BYCOVID_eu"
          options={{height: 660}}
        />
      </Card>
    </Col>*/}
  </Row>
</Container>

</div>
